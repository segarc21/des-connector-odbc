  ...
  ENV(SQLINTEGER ver) : odbc_ver(ver) {
    ...
    HANDLE handle_map_file_shmem =
        CreateFileMappingA(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0,
                           sizeof(SharedMemory), SHARED_MEMORY_NAME);

    if (handle_map_file_shmem == nullptr) {
      exit(1);  // TODO: handle this with SQL errors. Generally the problem is
                // the lack of admin privileges.
    }

    shmem = (SharedMemory *)MapViewOfFile(
        handle_map_file_shmem, FILE_MAP_ALL_ACCESS, 0, 0, sizeof(SharedMemory));
 ...
 public:
  ...
  DES_FIELD *get_DES_FIELD() {
    this->field->max_length =
        getMaxLength();  // we need to have this value updated right now
    return this->field;
  }

  Column(const std::string &table_name, const std::string &col_name,
         const Type &col_type, const SQLSMALLINT &col_nullable) {
...
class ResultTable {  // Internal representation of a result view.
 public:
  ...
  DES_ROWS *generate_DES_ROWS(const int current_row) {
    ...
    DES_ROWS *ptr = rows;
    int n_rows =
        columns[names_ordered[0]]
            .values
            .size();  // sloppy way to do this. TODO: guarantee encapsulation
...
struct STMT {
  ...
  STMT_OPTIONS stmt_options;
  std::string
      table_name;  // This value may be empty. Useful for SQLStatistics calls
...
inline void ResultTable::create_results_SQLGetTypeInfo() {
  ...
  for (int i = 0; i < supported_types.size(); i++) {
    if (type_requested == supported_types[i] ||
      ...
      for (auto pair : typestr_sqltype_map) {
        ...
        if (sql_data_type == type_requested ||
          ...
          // It seems odd that this interpretation of the SQL_DATETIME_SUB
          // column is correct.
          // TODO: research
