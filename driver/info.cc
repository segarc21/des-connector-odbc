...
# endif
  ...
  case SQL_KEYWORDS:
    ...
    */
    if (is_minimum_version(dbc->des->server_version, "8.0.22"))
      ...
      DESINFO_SET_STR("ANALYZE,BEFORE,BIGINT,BINARY,BLOB,CHANGE,COLUMNS,"
  ...
  case SQL_TXN_CAPABLE:
    if (trans_supported(dbc) && (!dbc->ds.opt_NO_TRANSACTIONS))
      DESINFO_SET_USHORT(SQL_TC_DDL_COMMIT);
    else
  ...
  case SQL_TXN_ISOLATION_OPTION:
    if (!trans_supported(dbc) || (dbc->ds.opt_NO_TRANSACTIONS))
      ...
      DESINFO_SET_ULONG(SQL_TXN_READ_COMMITTED | SQL_TXN_READ_UNCOMMITTED |
